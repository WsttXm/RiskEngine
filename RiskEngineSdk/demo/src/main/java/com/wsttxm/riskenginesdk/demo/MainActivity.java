...
public class MainActivity extends AppCompatActivity {
    ...
    private static final String SERVER_URL = "http://192.168.31.154:8080";
    private static final String APP_KEY = "e5b336402045489d9b6881f5ad48c796";
    private static final String ENCRYPTION_KEY_B64 = "aO67Yk4akGoLkDbmGpc8S84/ZpReuj9LYSS8dBChHFk=";

    private MaterialButton btnCollect;
    private MaterialButton btnCollectOnly;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnCollect.setOnClickListener(v -> doCollect(true));
        btnCollectOnly.setOnClickListener(v -> {
            RiskEngine.shutdown();
            initSdk(false);
            doCollect(false);
        });
    ...
    private void initViews() {
        ...
        btnCollectOnly = findViewById(R.id.btnCollectOnly);
    ...
    private void displayConfig() {
        ...
        TextView tvAppKey = findViewById(R.id.tvAppKey);
        TextView tvEncryption = findViewById(R.id.tvEncryption);

        tvServerUrl.setText("Server: " + SERVER_URL);
        tvAppKey.setText("AppKey: " + APP_KEY);
        tvEncryption.setText("Encryption: AES-256-GCM (enabled)");
    }

    private void initSdk(boolean withServer) {
        ...
        if (withServer) {
            builder.serverUrl(SERVER_URL)
                   .appKey(APP_KEY)
                   .encryptionKey(Base64.decode(ENCRYPTION_KEY_B64, Base64.NO_WRAP));
        }

        RiskEngine.init(this, builder.build());
    }

    private void doCollect(boolean willUpload) {
        if (collecting) {
        ...
        btnCollectOnly.setEnabled(false);
        ...
        tvUploadStatus.setText(willUpload ? "采集完成后将自动上报" : "仅本地采集, 不上报");
        tvUploadStatus.setTextColor(getColor(R.color.text_secondary));
        ...
        RiskEngine.collect(new RiskEngineCallback() {
            ...
            public void onSuccess(RiskReport report) {
                runOnUiThread(() -> {
                    ...
                    displayReport(report, elapsed, willUpload);
                    collecting = false;
                    btnCollect.setEnabled(true);
                    btnCollectOnly.setEnabled(true);

                    // Re-init with server for next collect
                    if (!willUpload) {
                        RiskEngine.shutdown();
                        initSdk(true);
                    }
    ...
    private void displayReport(RiskReport report, long elapsedMs, boolean uploaded) {
        // Risk Level
        ...
        // Upload status
        if (uploaded) {
            tvUploadStatus.setText("已上报至 " + SERVER_URL + " (加密传输)");
            tvUploadStatus.setTextColor(getColor(R.color.risk_safe));
        } else {
            tvUploadStatus.setText("仅本地采集, 未上报");
            tvUploadStatus.setTextColor(getColor(R.color.text_secondary));
        }
    ...
    private void displayRawJson(RiskReport report) {
        ...
        try {
            ...
            tvJsonData.setText(gson.toJson(parsed));
        } catch (Exception e) {
